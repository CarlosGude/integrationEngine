...
Hecho todo lo de abajo (24 puntos). Local: 643 tests, PHPStan max y php-cs-fixer en verde, tests
del landing en verde, y la demo compila y pasa sus tests contra este código. Queda confirmarlo con el
próximo análisis de SonarCloud. Decisiones que cambian respecto al plan:

- **S8546** (`composer update` en los workflows) y **S1144** (`dispatchBatch`): en vez de marcarlos a
  mano en SonarCloud, se anotan con `NOSONAR` y su motivo, así la decisión queda en el código.
- **S3415** (`CachingMiddlewareTest.php:237`): el assert ya estaba en orden (expected, actual).
  Sonar veía el `null` con el que se inicializaba la variable; ahora empieza en `[]`. Si el aviso
  sigue saliendo, es un falso positivo.
- **Landing**: además de las barras, 14 spans estaban escritos `class="str\"'…'`, sin el `>`, y el
  navegador no mostraba ese texto. Arreglados, con un guard en los tests.
...
- [x] Renombrar `sonar-project.properties` → `.sonarcloud.properties` (mismo contenido).
...
- [x] Cambiar a `hash('xxh128', …)` en los 2 de `src/`, que es un hash no criptográfico pensado
  para esto, y actualizar los tests.
- [x] Anotarlo en el CHANGELOG: cambia el formato de las claves, así que tras actualizar hay un
...
- [x] Generarlo con `random_bytes(16)`, ajustando los bits de versión y variante. No hacen falta
  dependencias nuevas.

### 1.3 Seguridad: workflows (3 issues)

- [x] `.github/workflows/contract.yml:28` (S7637): fijar `shivammathur/setup-php` al mismo SHA
  que `php.yml` (`7c071dfe9dc99bdf297fa79cb49ea005b9fcadbc` # 2.37.1).
- [x] `.github/workflows/contract.yml:40` y `.github/workflows/php.yml:101` (S8546,
...
- [x] `tests/Bundle/Generator/WebhookFileGeneratorTest.php:146`: cambiar `require` por
  `require_once` en el autoloader del test.

### 1.5 Duplicación

- [x] Se resuelve casi entera con el punto 0.
- [x] Lo que queda en `src/` es `writeFile()`, idéntico en `MakeWebhookCommand` (101-128) y
...
- [x] Escribir las barras como `\\`. Son parte de los 38 avisos S6535; el resto son `\"`
  innecesarios, que solo son cosméticos.
- [x] Añadir un guard en `landing/test/content-guards.test.js` que falle si el HTML renderizado
...
- [x] **S108 ×7, `catch` vacíos**: el commit `6b6798a` borró los comentarios que los
...
- [x] **S1068 ×2**: `IntegrationEngine::$logger` y `AuthenticationHandler::$integrationName` son
  propiedades promovidas que solo se usan en el constructor. Pasarlas a parámetros normales; la
  firma pública no cambia.
- [x] **S1481 ×2**: `src/Bundle/Generator/WebhookFileGenerator.php:66-67` tiene dos variables sin
  usar (`$headerName`, `$verifierType`).
- [x] **S1144**: `MiddlewareClient::dispatchBatch` es un **falso positivo**, porque se usa como
  first-class callable (`$this->dispatchBatch(...)`). Marcarlo así en SonarCloud.
- [x] **S1172**: `MultiPlatformWebhookController::ingest()` no usa `$platform`. Quitarlo o
  aceptarlo, ya que el controller está deprecated.
- [x] **S107**: `IntegrationCompilerPass::wireIntegration()` tiene 8 parámetros. Agrupar lo
  resuelto (mapa de adapters y middlewares registrados) o aceptarlo.
- [x] **S1142 ×3** (más de 3 `return`): `TimestampedHmacSignatureVerifier::verify`,
...
- [x] S1481 ×5: `tests/Infrastructure/YamlConfigAdapterTest.php` 44, 55, 66, 77, 92, `$_` sin
  usar.
- [x] S1172 ×2: `tests/Infrastructure/Cache/CachingMiddlewareTest.php` 311, 316, parámetro
  `$reqs` sin usar.
- [x] S3415: `tests/Infrastructure/Cache/CachingMiddlewareTest.php:237`, argumentos del assert
  invertidos (expected/actual).
- [x] S1185: `tests/Infrastructure/YamlConfigAdapterWebhooksTest.php:15`, `tearDown()` que solo
  llama al padre.
- [x] S112 ×2: `tests/Fake/FakeLogger.php:47` y `tests/Fake/WebhookMapperResolverAdapter.php:96`
...
- [x] Pasarlos a `assert.ok(...)` o `assert.doesNotMatch(...)` de `node:assert`.
